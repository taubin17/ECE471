...
int main(int argc, char **argv) {
	...
	while (1)
	{
		// Send data telling MCP3008 we want channel 2
		data_out_0[0] = 0x01;
		data_out_0[1] = 0xA0;
		data_out_0[2] = 0x00;
		...
		// Convert our voltage to temperature using deg_C = (100 * vin) -50
		temp = (100 * vin) - 50;
		

		// If it's wicked cold, tell em
		if (temp < 0) printf("It's freezing out. Bring a coat! The temperature was %lf *C\n", temp);
		
		// If it's wicked hot, tell em
		if (temp > 100) printf("It's boiling hot out! Literally! Enjoy melting at a temperature of %lf*C\n", temp);

		
		// If it's normal(ish), tell em
		else printf("The temperature is approximately: %lf*C\n", temp);
